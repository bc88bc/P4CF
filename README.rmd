...
- Lines 5 through (5 + *nseg*): Coordinates of the (*nseg* + 1) vertices which define the rupture trace. 
   ...
   - Strike of the rupture trace dictates the order in which its vertices are specified, and vice versa. 
     As an example, strike of segment $i$ is the azimuth from vertex *i* to vertex *i+1*.
...
- Line (5 + *nseg* + 1): (*Dip*(i), i = 1, *nseg*), *H_Trace*, *H_Top*, *H_Bot*
  ...
  - *H_Trace*: depth (in KM) of fault trace
    - *H_Trace* is deprecated; it is kept for the reason of backward compatibility 
...
- *staID*, *StaName*, *Sta_x*, *Sta_y* (a10, a55, 2f10.4)
  ...
  - (*Sta_x*, *Sta_y*), depending on the value of *inLatLon*, is either the Cartesian coordinate (X, Y) 
    or geographic coordinate (*Latitude, Longitude*) of the station
...
- Distances derived from (*U, T*)
  ...
  - $R_y = U - L/2$, strike-parallel distance to rupture trace
  - $R_{y0}$
...
- Strike-parallel direction 
  ...
  - *AveStrike*: 
    - If *method* = 1, the average strike-parallel direction, taken as the azimuth from the first vertex to the last vertex of user-specified rupture trace.
